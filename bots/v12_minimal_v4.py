"""Kaggriculture v12 MINIMAL v4: 1 goose + wheat FEED + melon (with placement)."""
...
def agent(obs):
    try:
        ...
        empty_coop = _find_empty_coop(f["tiles"])
        farmer_inv = invs[0] if invs else {}
        farmer_wheat = farmer_inv.get("WHEAT", 0)
        
        # Определяем что нужно делать фермеру
        # 1. Если гуся нет на поле — размещаем
        if goose_pos is None:
            # PICKUP из сарая
            if shed.get("GOOSE", 0) > 0 and _shed_adjacent(pos, board_size):
                actions_f = ["PICKUP", "GOOSE", 1]
            elif shed.get("GOOSE", 0) > 0:
                # Идём к сараю
                target = _nearest_shed(pos, board_size)
                actions_f = _step(pos, target) or ["PASS"]
            # BUILD_COOP если нет
            elif empty_coop is None:
                target = (2, 2)  # фиксированная позиция
                if pos == target:
                    actions_f = ["BUILD_COOP"]
                else:
                    actions_f = _step(pos, target) or ["PASS"]
            # PLACE GOOSE
            elif empty_coop is not None and pos == empty_coop:
                actions_f = ["PLACE", "GOOSE"]
            elif empty_coop is not None:
                actions_f = _step(pos, empty_coop) or ["PASS"]
            else:
                actions_f = ["PASS"]
        else:
            # Гусь на поле — обычный цикл
            tasks = _collect_tasks(f, day, seeds, target_harvest_age)
            
            # PICKUP WHEAT приоритет
