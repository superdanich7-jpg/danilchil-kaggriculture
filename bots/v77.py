"""Kaggriculture v77: full-field utilization (songxin formula). straw 20 + wheat 15 fills freed tiles (util 44% -> 65%+), wheat feeds the herd free, melon wave held to d15."""
...
def _pick_crop(day, seeds, counts, melon_quota, allow_straw=True, wheat_quota=15):
    ...
    if w > 0 and counts.get("WHEAT", 0) < wheat_quota and day <= 26:
        return "WHEAT"
    ...
    if m > 0 and counts.get("MELON", 0) < melon_quota and day < 15:
        return "MELON"
    return None
...
def agent(obs):
    try:
        ...
        wheat_quota = 12 if melon_mode else 15
