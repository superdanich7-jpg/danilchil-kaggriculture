...
def agent(obs):
    try:
        ...
        max_hands_today = 15 if day >= 11 else 8
        ...
        if n_hired < max_hands_today:
            if (n_hired < 7 and money - fib_cost >= FEED_RESERVE) \
        ...
        if day in (0, 1, 2, 3, 4, 5) and hour % 4 == 0:
            with open(r"c:\kaglab\dbg_scan.log", "a") as _dbg:
                _aw = [t.get("consecutive_unfed", -1) for _, t in anims]
                _af = [t.get("fed_today", False) for _, t in anims]
                _dbg.write(f"d={day} h={hour} money={money} herd={herd} "
                           f"in_shed={in_shed} shedW={shed.get('WHEAT', 0)} "
                           f"unfed={_aw} fed={_af} nhands={n-1} "
                           f"inv0={dict(invs[0]) if invs else {}}\n")
