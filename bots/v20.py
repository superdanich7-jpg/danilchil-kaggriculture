...
def _pick_crop(day, seeds, counts, melon_quota, allow_straw=True, wheat_quota=6):
    ...
    if m > 0 and counts.get("MELON", 0) < melon_quota and day < 18:
        return "MELON"
    # strawberry planted after day 19 yields only once - still profitable
...
def agent(obs):
    try:
        ...
        if (enemy_melons >= MELON_DANGER_THRESHOLD or enemy_straw >= 6
                or enemy_anim >= 8):
            _MODE_DIV["on"] = True
        melon_mode = False  # ITER28: permanent top-player hybrid
        melon_quota = 10
        cow_target = 10
        wheat_quota = 4 if melon_mode else 6
        herd_cap = 8 if melon_mode else 20
        if melon_mode:
        ...
        # needs ~1 wheat/animal/day. STOP on day 29: feeding has no next-day
        # payoff and the buy->sell churn vs wheat-surplus sale wastes spread ---
        if day < 29 and (herd > 0 or in_shed > 0) \
                and shed.get("WHEAT", 0) < feed_need and money > 90:
            market.append(["BUY_PRODUCT", "WHEAT", min(10, max(2, herd))])
        # --- land: after first cash flow, mode-dependent depth.
        # NOTE (ITER24): 3rd quadrant in melon_mode measured NET NEGATIVE
        # (seed flow / watering cannot service 40 tiles -> one weak wave),
        # so it stays diversified-only. ---
        if len(quads) == 1 and day >= 5 and money > 1500 and in_shed == 0:
        ...
        # ITER28 template: cows first (fast ROI), then sheep; money > 800
        if 9 <= day <= 22 and herd < 20 and in_shed == 0 and money > 900 \
                and shed.get("WHEAT", 0) >= 2:
            n_sh = sum(1 for _, t in anims if t.get("animal") == "SHEEP")
            n_cow = sum(1 for _, t in anims if t.get("animal") == "COW")
            if n_cow < 6 and day <= 16:
                market.append(["BUY_ANIMAL", "COW", 1])
                if money > 1600 and n_cow < 5:
                    market.append(["BUY_ANIMAL", "COW", 1])
            elif n_sh < 10:
