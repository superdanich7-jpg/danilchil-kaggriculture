...
def _pick_crop(day, seeds, counts, melon_quota, allow_straw=True, wheat_quota=6):
    ...
    if w > 0 and counts.get("WHEAT", 0) < wheat_quota:
        return "WHEAT"
    if m > 0 and counts.get("MELON", 0) < melon_quota:
        return "MELON"
    if day >= 12 or not m:
        if s > 0 and allow_straw:
            return "STRAWBERRY"
    return None
...
def agent(obs):
    try:
        ...
        if herd <= 4:
            ...
            n_anim_hands = 5
        enemy = _scan_opponent_crops(obs)
        harvest_age = 11 if enemy >= MELON_DANGER_THRESHOLD else 12
        # adaptive anti-correlation: vs a weak/non-melon opponent the shared
        # market stays under I0 -> melon price HOLDS/RISES, go melon-monopoly;
        # vs a melon-flooder diversify into strawberry (price holds at ~169)
        melon_mode = enemy < 8
        melon_quota = 20 if melon_mode else 13
        wheat_quota = 4 if melon_mode else 6
        herd_cap = 6 if melon_mode else 20
        if melon_mode:
        ...
        if shed.get("WOOL", 0) > 0:
            market.append(["SELL", "WOOL", min(6, shed["WOOL"])])
        if shed.get("MILK", 0) > 0:
            market.append(["SELL", "MILK", min(8, shed["MILK"])])
        if shed.get("STRAWBERRY", 0) > 0:
            market.append(["SELL", "STRAWBERRY", min(8, shed["STRAWBERRY"])])
        if day >= 12 and shed.get("MELON", 0) > 0:
            market.append(["SELL", "MELON", min(10, shed["MELON"])])
        fert_res = 0 if (day < 10 or melon_mode) else FERT_RESERVE
        fv = shed.get("FERTILIZER", 0) - fert_res
        if fv > 0:
            market.append(["SELL", "FERTILIZER", min(8, fv)])
        ...
        fib_cost = (1, 1, 2, 3, 5, 8, 13)[min(n_hired, 6)]
        if n_hired < MAX_HANDS and money > fib_cost:
            market.append(["HIRE"])
        ...
        if day == 0 and hour == 0 and seeds.get("MELON", 0) == 0:
            market += [["BUY_ANIMAL", "COW", 1], ["BUY_ANIMAL", "SHEEP", 1],
                       ...
                       ["BUY_PRODUCT", "WHEAT", 4]]
        # --- seed top-ups (deferred: solvency until melon day-12 payoff) ---
        if melon_mode:
            # melon monopoly: keep the field fully seeded
            if seeds.get("MELON", 0) < 4 and money > 330:
                market.append(["BUY_SEED", "MELON", 4])
        else:
            if day >= 5 and seeds.get("STRAWBERRY", 0) < 2 and money > 700:
                market.append(["BUY_SEED", "STRAWBERRY", 2])
            if day < 11 and seeds.get("MELON", 0) < 1 and money > 150:
                market.append(["BUY_SEED", "MELON", 2])
        # --- feed logistics: buy wheat (log impact ~0.2, cheap); a big herd
        # needs ~1 wheat/animal/day -> scale purchases with herd size ---
        feed_need = max(WHEAT_FEED_MIN, herd)
        if (herd > 0 or in_shed > 0) and shed.get("WHEAT", 0) < feed_need \
                and money > 90:
            market.append(["BUY_PRODUCT", "WHEAT", min(10, max(2, herd))])
        # --- land: after first cash flow, mode-dependent depth ---
        if len(quads) == 1 and day >= 5 and money > 1500:
            market.append(["BUY_LAND"])
        if len(quads) == 2 and money > (4000 if melon_mode else 2600):
            market.append(["BUY_LAND"])
        if len(quads) == 3 and not melon_mode and money > 9000:
            market.append(["BUY_LAND"])
        # --- herd scaling: a cared-for sheep returns ~300/day -> buy fast;
        # stop buying after day 24 (no time to repay) ---
        if 9 <= day <= 24 and herd < herd_cap and in_shed == 0 and money > 900 \
