"""Kaggriculture v12 debug v2: animals + wheat + melon on v9 base (front-running)."""
...
def _collect_tasks(f, day, seeds, animals, buildings, harvest_age_melon):
    ...
    if day < 5:
        # Сначала строим постройки
        for bid, b in buildings.items():
            if not b.get("built"):
                bx, by = b["pos"]
                tasks.append(("BUILD_COOP" if b["type"] == "COOP" else "BUILD_PASTURE", bx, by, {"building_id": bid, "type": b["type"]}))
        # Затем размещаем животных (если постройка уже есть)
...
def agent(obs):
    try:
        ...
        # Покупка животных (market-ордер)
        if day < 5 and hour == 0:
            for aid, a in animals.items():
                if not a.get("purchased") and f["money"] >= ANIMAL_PRICES[a["type"]]:
                    market.append(["BUY_ANIMAL", a["type"], 1])
        ...
        if hour == 0:
            ...
            build_orders = [m for m in market if m[0] == "BUILD"]
            hire_orders = [m for m in market if m[0] == "HIRE"]
            
            log_line = (day, f["money"], animal_count, consec_unfed, wheat_shed, 
                       wheat_inv, feeds_done, shed_total, buy_animal_orders, build_orders, hire_orders)
